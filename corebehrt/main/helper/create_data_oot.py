...
def create_and_save_features(cfg, splits, logger) -> None:
    ...
    """
    logger.info(f"🚨 DEBUG - splits received: {splits}")
    combined_patient_info = pd.DataFrame()
  
    for split_name in splits:
        logger.info(f"Creating features for {split_name}")
        path_name = os.path.join(cfg.paths.data, split_name)

        if not os.path.exists(path_name):
            logger.warning(f"Split '{split_name}' skipped: folder not found at {path_name}")
            continue

        split_save_path = os.path.join(cfg.paths.features, split_name)
        os.makedirs(split_save_path, exist_ok=True)
        shards = [
            shard for shard in os.listdir(path_name)
            if not shard.startswith(".") and os.path.isfile(os.path.join(path_name, shard))
        ]

        if len(shards) == 0:
            logger.warning(f"No shards found for split '{split_name}', skipping...")
            continue
        ...
        for shard_idx, shard in enumerate(shards, 1):
            ...
            ##
            # Remove Pationts without DOB
            before = set(concepts["subject_id"])
            has_dob = concepts[concepts["code"] == "DOB"]["subject_id"].unique()
            concepts = concepts[concepts["subject_id"].isin(has_dob)]
            after = set(concepts["subject_id"])
            removed = before - after
            logger.info(f"### Removed {len(removed)} patients without DOB")
            ##
    ...
    ###
    if not combined_patient_info.empty:
        ...
        logger.warning(" No patient info generated; all splits were empty or missing.")
