...
##
    logger.info("Extracting representations for BEST model...")
    trainer._extract_and_save_representations(epoch)
...
def log_best_metrics(loss: float, metrics: dict, split: str) -> None:
    ...
    """
    # تخت کردن دیکشنری متریک‌ها:
    flat_metrics = flatten_dict(metrics)

    # اضافه کردن loss به بقیه متریک‌ها:
    row = {f"{split}_loss": loss, **flat_metrics}

    # ساخت کلیدهای prefixed برای لاگ:
    prefixed = {f"best.{split}.{k}": v for k, v in row.items() if isinstance(k, str)}
